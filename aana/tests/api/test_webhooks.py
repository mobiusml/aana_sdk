...
def test_register_webhook(db_session):
    payload = {
        "user_id": "user_123",
    ...
    assert response.json() == {"message": "Webhook registered successfully"}

    webhook_repo = WebhookRepository(db_session)
    webhooks = webhook_repo.get_webhooks("user_123")
    assert len(webhooks) == 1
    assert webhooks[0].webhook_url == "https://example.com/task-updates"
    assert webhooks[0].events == ["task.completed", "task.failed"]
