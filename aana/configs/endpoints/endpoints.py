...
# find all json files in the endpoints directory
# for each file, load the json and create an endpoint object

endpoints = {}
endpoint_files = glob(str(resources.path("aana.configs.endpoints", "*.json")))

for file in endpoint_files:
    pipeline_name = Path(file).stem
    endpoints[pipeline_name] = []
    with open(file, "r") as f:
        data = json.load(f)
        for endpoint in data:
            outputs = []
            for output in endpoint["outputs"]:
                outputs.append(EndpointOutput(**output))
            endpoints[pipeline_name].append(
                Endpoint(
                    name=endpoint["name"],
                    path=endpoint["path"],
