...
class MediaRepository(BaseRepository[M]):
    ...
    def __init__(self, session: AsyncSession, model_class: type[M] = MediaEntity):
        """Constructor."""
        super().__init__(session, model_class)

    async def check_media_exists(self, media_id: MediaId) -> bool:
    ...
    async def create(self, entity: MediaEntity) -> MediaEntity:
        ...
        if await self.check_media_exists(entity.id):
