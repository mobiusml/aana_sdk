...
class ASRPostProcessingForDiarization:
    ...
    def __init__(
        ...
        """Initializes the ASRPostProcessingForDiarization class with diarization and transcription segments.
    ...
    def process(self) -> list[AsrSegment]:
        ...
        if not self.transcription_segments or not self.diarized_segments:
            return self.transcription_segments

        # 1. Assign speaker labels to each segment and word
        speaker_labelled_transcription = self._assign_word_speakers(
            self.diarized_segments, self.transcription_segments
        )

        # 2. Align speakers with punctuation
        word_speaker_mapping = self._align_with_punctuation(
            speaker_labelled_transcription
        )

        # 3. Create new transcription segments with speaker information
        segments = self._create_speaker_segments(word_speaker_mapping)

        # 4. Assign confidence variables to the new segments
        segments = self._add_segment_variables(segments, self.transcription_segments)

        # Optional: Merge consecutive speaker segments
        if self.merge:
            segments = self._combine_homeogeneous_speaker_asr_segments(segments)
    ...
    def _assign_word_speakers(
        ...
        for segment in transcription_segments:
            ...
            segment.speaker = self._get_speaker_for_interval(
                diarized_segments,
            ...
            if segment.words:
                for word in segment.words:
                    word.speaker = self._get_speaker_for_interval(
                        diarized_segments,
    ...
    def _find_nearest_speaker(
        self, index: int, word_speaker_mapping: list[AsrWord], reverse: bool = False
    ) -> str | None:
    ...
    def _align_with_punctuation(
        self, transcription_segments: list[AsrSegment], max_words_in_sentence: int = 50
    ) -> list[AsrWord]:
        ...
        for i, item in enumerate(word_speaker_mapping):
            if item.speaker is None:
                item.speaker = self._find_nearest_speaker(
        ...
        while k < len(word_speaker_mapping):
            if (
                ...
                right_idx = self._get_last_word_idx_of_sentence(
    ...
    def _create_new_segment(
        self, word_info: AsrWord, speaker: str | None, is_empty: bool = False
    ) -> AsrSegment:
    ...
    def _create_speaker_segments(
        self, word_list: list[AsrWord], max_words_per_segment: int = 50
    ) -> list[AsrSegment]:
        ...
        for word_info in word_list:
            ...
            if speaker != current_speaker:
                ...
                current_segment = self._create_new_segment(word_info, speaker)
                ...
                if current_segment:
                    ...
                    (
                        current_segment,
                        word_count,
                    ) = self._split_segment_on_length_punctuation(
                        current_segment,
                        word_info,
                        word_count,
    ...
    def _add_segment_variables(
        self, segments: list[AsrSegment], transcription_segments: list[AsrSegment]
    ) -> list[AsrSegment]:
        ...
        for segment in segments:
            confidence, no_speech_confidence = self._determine_major_segment_confidence(
    ...
    def _determine_major_segment_confidence(
        self, segment: AsrSegment, transcription_segments: list[AsrSegment]
