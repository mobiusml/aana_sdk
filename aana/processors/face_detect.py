...
class SCRFD:
    ...
    def __init__(self, inference_backend: ONNXModel, ver=1):
    ...
    def prepare(self, nms_treshold: float = 0.4, **kwargs):
        ...
        max_prop_len = self._get_max_prop_len(
            self.input_shape, self._feat_stride_fpn, self._num_anchors
        )
    ...
    def detect(self, imgs, threshold=0.5):
        ...
        for e in range(self.infer_shape[0]):
            det, kpss = filter(  # This is where NMS is performed
    ...
    def _forward(self, blob):
        ...
        t0 = time.time()
        if self.stream:
            net_outs = self.session.run(from_device=True, infer_shape=self.infer_shape)
        else:
    ...
    def _postprocess(self, net_outs, input_height, input_width, threshold):
        ...
        bboxes, kpss, scores = self._process_strides(
            net_outs, threshold, anchor_centers
        )
...
# @njit(fastmath=True, cache=True)
# def generate_proposals(score_blob, bbox_blob, kpss_blob, stride, anchors, threshold, score_out, bbox_out, kpss_out, offset):
#     total = offset
#     for ix in range(0, anchors.shape[0]):
#         if score_blob[ix, 0] > threshold:
#             score_out[total] = score_blob[ix]
...
# @njit(fastmath=True, cache=True)
# def facedetect_filter(bboxes_list: np.ndarray, kpss_list: np.ndarray, scores_list: np.ndarray, nms_threshold: float = 0.4):
#     pre_det = np.hstack((bboxes_list, scores_list))
#     keep = nms(pre_det, thresh=nms_threshold)  # see helpers.py
#     keep = np.asarray(keep)
#     det = pre_det[keep, :]
#     pred_scores = pre_det[keep, -1]  # Box confidence scores
#     kpss = kpss_list[keep, :]
#     kpss = kpss.reshape((kpss.shape[0], -1, 2))
#     return det, kpss, pred_scores
...
#         self.input_shape = [self.batch_size, ] + self.config['input_shape']
#         self.center_cache = {}
...
#                 input_height, input_width, self._feat_stride_fpn, self._num_anchors)
#         anchor_centers = self.center_cache[key]
#         bboxes_by_img, kpss_by_img, scores_by_img = self._process_strides(
#             net_outs, threshold, anchor_centers)

#         # NMS
#         dets_list, kpss_list, pred_scores = [], [], []
#         for e in range(batch_size):
#             det, kpss, score = facedetect_filter(
#                 bboxes_by_img[e], kpss_by_img[e], scores_by_img[e], self.nms_threshold)
#             dets_list.append(det)
#             kpss_list.append(kpss)
#             pred_scores.append(score)
#         return dets_list, kpss_list, pred_scores

#     def _build_anchors(self, input_height, input_width, strides, num_anchors):
#         centers = []
#         for stride in strides:
#             height = input_height // stride
#             width = input_width // stride
